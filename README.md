...
3. **Add the resource to your dashboard:**
	```yaml
	resources:
	  - url: /hacsfiles/rotating-chart-card/dist/rotating-chart-card.js
		 type: module
	```
4. Add the card to your Lovelace dashboard.
	- **Note:** When installed via HACS, the file is served from `/hacsfiles/rotating-chart-card/dist/rotating-chart-card.js`. This ensures compatibility with HACS upgrades and file management.
...
2. Place it in your `/config/www/dist/` folder (create the `dist` folder if it doesn't exist).
3. Add the resource to your dashboard:
	 ```yaml
	 resources:
		 - url: /local/dist/rotating-chart-card.js
			 type: module
	 ```
4. Add the card to your Lovelace dashboard.
